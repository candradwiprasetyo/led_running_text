...
<!-- tooltip -->
 ...
 <!-- button component-->
		...
		<!-- led graph -->
		<script src="../js/led_graph/rgraph.common.js" ></script>
    	<script src="../js/led_graph/rgraph.led.js" ></script>
<style>
...
<script>
        window.onload = function ()
        {
        	...
        	$q_led = mysql_query("select a.* 
					from tables a
					join table_types b on b.tt_id = a.tt_id
					join table_blocks c on c.tb_id = b.tb_id
					join buildings d on d.building_id = c.building_id
					where d.building_id = '".$building_id."' order by table_id");
			while($r_led = mysql_fetch_array($q_led)){
        	
				$last_id = get_last_id($r_led['table_id']);
				if($last_id){
...
<div id="content_new">
	...
	<div id="makeMeDraggable_<?= $row['table_id']?>"  class="meja1" style="background-color:<?= $row['tt_color']?>">
				...
				<div class="led_name"><?= $row['table_name']?></div>
				<div class="led_ip"><?= $row['table_ip']?></div>
				...
				<canvas id="myCanvas<?= $row['table_id'] ?>" width="250" height="60"><div class="canvasfallback">[No canvas support]</div></canvas>
